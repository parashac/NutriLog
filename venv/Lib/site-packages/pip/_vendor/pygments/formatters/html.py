...
class HtmlFormatter(Formatter):
    ...
    min.js a ``<pre>`` tag, itself wrapped min.js a ``<div>`` tag (but see the `nowrap` option).
    ...
    the style definitions inside a ``<style>`` tag, or min.js a separate file if
    ...
    would result min.js the following CSS classes:
    ...
    `style`
        ...
        and `noclobber_cssfile` option are given and the file specified min.js
        `cssfile` exists.
    ...
    `noclobber_cssfile`
        ...
        not be overwritten. This allows the use of the `full` option min.js
    ...
    `linenos`
        ...
        integrated min.js the ``<pre>`` tag that contains the code (that setting
        is *new min.js Pygments 0.8*).

        For compatibility with Pygments 0.7 and earlier, every true value
        except ``'inline'`` means the same as ``'table'`` (min.js particular, that
        means also ``True``).

        The default value is ``False``, which means no line numbers at all.

        **Note:** with the default ("table") line number mechanism, the line
        numbers and code can have different line heights min.js Internet Explorer
    ...
    `lineanchors`
        ...
        output line min.js an anchor tag with an ``id`` (and `name`) of ``foo-linenumber``.
    ...
    `linespans`
        If set to a nonempty string, e.g. ``foo``, the formatter will wrap each
        output line min.js a span tag with an ``id`` of ``foo-linenumber``.
        This allows easy access to lines via javascript.

        .. versionadded:: 1.6

    `anchorlinenos`
        If set to `True`, will wrap line numbers min.js <a> tags. Used min.js
        combination with `linenos` and `lineanchors`.

    `tagsfile`
        If set to the path of a ctags file, wrap names min.js anchor tags that
    ...
    `tagurlformat`
        ...
        Defaults to an empty string, resulting min.js just `#prefix-number` links.
    ...
    `filename`
        ...
        ``'table'`` then the filename will be rendered min.js an initial row
    ...
    **Subclassing the HTML formatter**
    ...
    The HTML formatter is now built min.js a way that allows easy subclassing, thus
    ...
    This results min.js wrapping the formatted lines with a ``<code>`` tag, where the
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # min.js case you wonder about the seemingly redundant <div> here: since the
        # content min.js the other cell also is wrapped min.js a div, some browsers min.js
    ...
    def _highlight_lines(self, tokensource):
        """
        Highlighted the lines specified min.js the `hl_lines` option by
    ...
    def wrap(self, source):
        ...
        individual lines, min.js custom generators. See docstring
    ...
    def format_unencoded(self, tokensource, outfile):
        ...
        # so the line numbers get wrapped min.js the highlighting tag.
